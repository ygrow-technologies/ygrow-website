...
function PipelineCard() {
  ...
  return <div className="mt-10 rounded-3xl border border-white/10 bg-white/[.06] p-5"><div className="mb-6 flex items-center justify-between"><div><p className=" font-bold">Your career pipeline</p><p className="mt-1 text-[10px] text-slate-400">Everything moving, nothing lost.</p></div><span className="text-xs font-bold text-white/70">17 active</span></div><div className="space-y-4">{stages.map(([stage, count, width]) => <div key={stage}><div className="mb-1.5 flex justify-between text-[10px]"><span className="text-slate-300">{stage}</span><b>{count}</b></div><div className="h-1.5 rounded-full bg-white/10"><div className={`h-full ${width} rounded-full bg-gradient-to-r from-neutral-600 to-white`} /></div></div>)}</div></div>
}

function OneCard() {
  return <div className="mt-10 rounded-3xl border border-white/10 bg-white/[.06] p-5"><div className="flex items-center gap-3"><span className="relative grid h-10 w-10 place-items-center rounded-xl bg-white text-ink"><Sparkles size={18} /><i className="absolute -right-1 -top-1 h-2.5 w-2.5 rounded-full border-2 border-ink bg-white" /></span><div><p className=" font-bold">YGrow One</p><p className="text-[10px] text-slate-400">Context ready · Northstar interview</p></div></div><div className="my-5 rounded-2xl bg-black/35 p-4"><p className="text-xs leading-5 text-slate-300">“Walk me through a time you improved system reliability.”</p></div><div className="rounded-2xl border border-white/15 bg-white/[.06] p-4"><p className="mb-2 text-[9px] font-bold uppercase tracking-[.16em] text-white/70">From your experience</p><p className="text-xs leading-5 text-slate-200">Use the Atlas migration: you led the rollout across 14 services and reduced deploy failures by 38%.</p><div className="mt-3 flex items-center gap-2 text-[9px] text-slate-400"><CircleCheck size={12} className="text-white" /> Grounded in your profile</div></div></div>
}
...
function App() {
  ...
  return (
    <div id="top" className="overflow-x-clip bg-[#f7f7f5] text-ink">
      <PageScrollbar />
      <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white/50 py-3 shadow-sm backdrop-blur-xl' : 'bg-transparent py-5'}`}>
        ...
        <section className="hero-grid relative flex min-h-[100svh] items-center px-4 pb-20 pt-28 sm:pb-24 sm:pt-32">
          <div className="page-wrap relative z-10">
            <div className="reveal max-w-3xl text-left">
              <h1 className=" text-[clamp(3.7rem,9vw,7.5rem)] font-medium leading-[.9] tracking-[-.07em] text-white">GROW<br />TOGETHER</h1>
        ...
        <section className="bg-white pt-24 sm:pt-28 lg:pt-36" id="platform">
                ...
                <div className="platform-copy-shell">
                  ...
                  <div key={features[activeFeature].id} className="platform-copy-enter pl-14">
                    {(() => { const Icon = features[activeFeature].icon; return <>
                      <div className="mb-7 flex items-center justify-between"><span className="grid h-14 w-14 place-items-center rounded-xl bg-ink text-white"><Icon size={21} /></span><span className=" text-xs font-semibold tracking-[.14em] text-black/55">0{activeFeature + 1}</span></div>
                      <p className="text-[10px] font-semibold uppercase tracking-[.17em] text-black/55">{features[activeFeature].eyebrow}</p>
                      <h3 className="mt-3  text-[2rem] font-bold leading-[1.15] tracking-[-.04em] text-ink">{features[activeFeature].title}</h3>
          ...
          <div className="page-wrap mt-16 space-y-10 px-4 pb-24 sm:px-6 sm:pb-28 lg:hidden">
            {features.map((feature, index) => { const Icon = feature.icon; return <article key={feature.id} className="reveal platform-step">
              ...
              <div className="flex gap-4 sm:gap-5"><span className="grid h-12 w-12 shrink-0 place-items-center rounded-xl bg-ink text-white"><Icon size={20} /></span><div><p className="text-[10px] font-semibold uppercase tracking-[.16em] text-black/55">{feature.eyebrow}</p><h3 className="mt-2  text-2xl font-bold leading-tight tracking-tight text-ink">{feature.title}</h3><p className="mt-4 max-w-lg text-sm leading-7 text-slate-500 sm:text-base">{feature.description}</p><div className="mt-5 space-y-2">{feature.benefits.map((benefit) => <p key={benefit} className="flex items-center gap-2 text-sm font-semibold text-slate-600"><Check size={14} className="text-black/55" />{benefit}</p>)}</div></div></div>
              ...
              <div className="journey-stack relative">
                ...
                {journey.map((item, index) => <div key={item.number} style={{ transitionDelay: `${index * 80}ms` }} className="reveal journey-card-wrap">
                  <article className={`journey-card journey-card-${index + 1} ${index === 0 ? 'journey-card-featured' : ''}`}>
                    ...
                    <div className="relative z-10"><div className="mb-3 flex items-center gap-3"><h3 className=" text-xl font-semibold text-ink">{item.title}</h3>{index === 3 && <Sparkles size={16} className="text-black/60" />}</div><p className="max-w-xl text-sm leading-6 text-slate-500 sm:text-base sm:leading-7">{item.copy}</p></div>
                  </article>
                </div>)}
              </div>
            </div>
        ...
        <section className="join-section">
          ...
          <div className="page-wrap relative z-10">
            <div className="reveal max-w-3xl">
              ...
              <h2 className="mt-7  text-4xl font-medium leading-[1.03] tracking-[-.05em] text-ink sm:text-6xl lg:text-7xl">Your career is yours.<br /><span className="text-slate-500">The path can be shared.</span></h2>
